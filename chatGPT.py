...
for topic in topics:
	...
	m.append({"role": "assistant", "content": assistant_reply})

	query = {"role": "user", "content": f'''
        Translate the above conversation to {language}.
		- Ensure that the conversation is realistic.
		- Whenever {language} speakers speak, they tend to use certain english words, so these must be maintained.
		...
		- Just output the required conversation without explaination.
        '''}
	
	m.append(query)
	response = client.chat.completions.create(model="o4-mini", messages=m)
	assistant_reply = response.choices[0].message.content.strip()
	print(f"Translation Response Length: {len(assistant_reply)}")
	m.append({"role": "assistant", "content": assistant_reply})
	assistant_reply = '\n'.join([line.strip() for line in assistant_reply.split('\n') if len(line.strip()) > 0])
	...
	query = {"role": "user", "content": f'''
		For the previosly generated conversation, please generate speaker information. Generate it in the following format:
	...
	query = {"role": "user", "content": f'''
		For the previously generated conversation, please generate conversation information. Generate it in the following
